...
open class KmbedGradlePlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        pluginManager.withPlugin(KMP_PLUGIN_ID) { applyWithKmp(project) }
    }

    private fun applyWithKmp(project: Project) {
        val extension = createProjectExtension(project)
        project.kmpExtension.compilerOptions.configureCompiler()
        ...
        project.afterEvaluate {
            ...
            val (generateSourcesCommonTask, generateSourcesCommonTestTask) = registerGenerateSourcesCommonTasks(
                project, extension
            )
            ...
            for (resourceSet in extension.resourceSets) {
                ...
                val processResourcesName = getProcessResourcesTaskName(compilation)
    ...
    private fun createProjectExtension(project: Project): KmbedProjectExtension {
    ...
    private fun getProcessResourcesTaskName(compilation: KotlinCompilation<*>): String {
        return if ("main" in compilation.name.lowercase()) "${compilation.target.name}ProcessResources"
        else "${compilation.target.name}${compilation.name.capitalized()}ProcessResources"
    }

    private fun KotlinCommonCompilerOptions.configureCompiler() {
        // Always enable expect-actual classes for all compilations
        freeCompilerArgs.add("-Xexpect-actual-classes")
    }

    private fun registerGenerateSourcesCommonTasks(
        project: Project, extension: KmbedProjectExtension
    ): Pair<TaskProvider<KmbedGenerateCommonSourcesTask>, TaskProvider<KmbedGenerateCommonSourcesTask>> {
    ...
    private fun registerGenerateSourcesCommonTask(
        ...
        val outputDir = if (isTest) extension.generatedCommonTestSourceDirectory
        else extension.generatedCommonSourceDirectory
        sourceSet.kotlin.srcDir(outputDir) // Add generated common sources to the same source set

        return project.tasks.register(
            "kmbedGenerateSourcesCommon$suffix", KmbedGenerateCommonSourcesTask::class.java
        ) { task ->
            task.group = TASK_GROUP
    ...
    private inline fun registerGenerateSourcesTask(
        ...
        val isTest = "test" in compilation.name.lowercase()
        val commonSourceSetName =
            if (isTest) extension.commonTestSourceSetName.get() else extension.commonSourceSetName.get()
        val commonSourceSet = project.kmpExtension.sourceSets.getByName(commonSourceSetName)
        val commonResourceDirectories = commonSourceSet.resources.srcDirs.toTypedArray()
